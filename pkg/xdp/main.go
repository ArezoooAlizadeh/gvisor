...
func main() {
	...
	if !ok {
		log.Fatalf("unknown program %q\n", *program)
	}

	// Load into the kernel. Note that this is usually done bpf2go, but
	// since we haven't set up that tool, we do everything manually.
